...
show_help() {
    echo "CANable 2.5 Slcan 固件一键编译脚本"
    ...
    echo
    echo "Examples:"
    echo "  $0                      # 编译 Slcan (MksMakerbase)"
    echo "  $0 -b Openlightlabs     # 编译 Slcan (Openlightlabs)"
    echo "  $0 --flash              # 编译并烧录"
    echo "  $0 --clean              # 清理"
}
...
MAKEFILE="Make_G431_Slcan_${BOARD}"

if [[ ! -f "${FIRMWARE_DIR}/${MAKEFILE}" ]]; then
    echo "错误: Makefile 不存在: ${MAKEFILE}"
    echo "支持的目标板: MksMakerbase, Openlightlabs"
    exit 1
fi
...
BUILD_DIR="Build_STM32G431xx_Slcan_${BOARD}"
TARGET_FILE="${BUILD_DIR}/STM32G431_Slcan2.5_${BOARD}"
